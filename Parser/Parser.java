...
public class Parser {
    ...
    void error(String s){ throw new Error("cerca de la linea "+lex.line+": "+s);}
    ...
    Stmt stmt() throws IOException{
        ...
        switch(look.tag){
            ...
            case ';':
                move();
                return Stmt.Null;
            /////////// FOR QUE FUNCIONA MAL :DDD 
                
            case Tag.FOR:
                For fornode = new For();
                savedStmt = Stmt.Enclosing; Stmt.Enclosing = fornode;
                match(Tag.FOR); match('('); x = bool(); match(';'); x2 = bool(); match(';');x3 = bool(); match(')');
                s = stmt();
                fornode.init(x, x2, x3, s);
                Stmt.Enclosing = savedStmt;
                return fornode;
                
            ////////// FOR QUE FUNCIONA MAL :cc
            case Tag.IF:
    ...
    Expr factor() throws IOException{
        ...
        switch(look.tag){
            ...
            default:
                error("error de sintasis");
                return x;
            case Tag.ID:
                String s = look.toString();
                Id id = top.get(look);
                if (id == null) error(look.toString() + " no declarado");
                move();
